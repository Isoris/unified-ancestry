// =============================================================================
// fst_outlier_scan.c — genome-wide FST outlier scan, cichlid-paper style.
//
// Distinct from inversion-candidate confirmation (which compares a known
// interval against local flanks). This binary scans the whole genome for
// SMALL highly-differentiated regions de novo.
//
// Pipeline (per the African cichlid paper, parameterizable):
//   1. Read per-window FST for an A-vs-B comparison.
//   2. Flag windows where FST >= --fst_threshold  (default 0.25 OR top_pct).
//   3. Sort by chrom/start; merge flagged windows whose gap on the same chrom
//      is <= --merge_gap_bp (default 10000) → outlier regions.
//   4. Optional HDR escalation: a merged region becomes "HDR" iff it contains
//      at least one window from a stronger-windows TSV (e.g. 10-kb fixed
//      windows) with FST >= --stronger_threshold (default 0.30). If no
//      stronger-windows TSV is given, fall back to max_FST(region) >=
//      stronger_threshold.
//   5. Optional overlap annotation against BED files: inversion candidates,
//      breakpoints, TE hotspots; nearest-gene from a genes TSV.
//
// Outputs:
//   --out_windows   <f>   flagged input windows (above fst_threshold)
//   --out_regions   <f>   merged regions, class = OUTLIER or HDR
//   --out_hdr       <f>   HDR-class regions only
//   --out_summary   <f>   JSON summary
//
// Caveat phrasing (matches user spec): the engine emits "empirical FST
// outliers" / "highly differentiated regions" — not "selected regions".
// Calibrated selection scans require simulation/permutation backgrounds and
// are not in scope here.
//
// Compile: gcc -O3 -march=native -Wall -o fst_outlier_scan fst_outlier_scan.c -lm
// =============================================================================
...
#define SCHEMA_VERSION "fst_outlier_scan_v1"
...
static int tsv_load(const char* path, Tsv* t) {
    FILE* f = fopen(path, "r");
    if (!f) { fprintf(stderr, "[fst_scan] Cannot open %s\n", path); return 0; }
...
typedef struct {
    ...
    long  start, end;
    double fst;
    long   n_variants;     // -1 if not provided
    int    flagged;        // 1 if fst >= threshold
} Window;

static int win_cmp(const void* a, const void* b) {
    const Window* wa = (const Window*)a;
    const Window* wb = (const Window*)b;
...
static int bed_load(const char* path, BedRec** out) {
    *out = NULL;
    if (!path) return 0;
    FILE* f = fopen(path, "r");
    if (!f) { fprintf(stderr, "[fst_scan] Cannot open BED %s\n", path); return 0; }
    int cap = 1024, n = 0;
    ...
    free(line); fclose(f);
    qsort(arr, (size_t)n, sizeof(BedRec), (int(*)(const void*, const void*))win_cmp); // chrom + start
    *out = arr;
    return n;
}
...
// True iff [s, e] on chrom overlaps any record in bed[].
...
// Nearest gene from a TSV: chrom<TAB>start<TAB>end<TAB>gene_id (header optional).
...
static int genes_load(const char* path, GeneRec** out) {
    *out = NULL;
    if (!path) return 0;
    FILE* f = fopen(path, "r");
    if (!f) { fprintf(stderr, "[fst_scan] Cannot open genes TSV %s\n", path); return 0; }
    int cap = 1024, n = 0;
...
typedef struct {
    ...
    double fst_sum, fst_min, fst_max;
    double* fst_values;
    int  fst_cap;
    int  contains_strong_window;   // from 10-kb fixed-window FST scan or fallback
} Region;

static void region_push_fst(Region* r, double v) {
    if (r->n_windows == r->fst_cap) {
        r->fst_cap = r->fst_cap ? r->fst_cap * 2 : 16;
        r->fst_values = (double*)realloc(r->fst_values, (size_t)r->fst_cap * sizeof(double));
    }
    r->fst_values[r->n_windows++] = v;
    r->fst_sum += v;
    if (v < r->fst_min) r->fst_min = v;
    if (v > r->fst_max) r->fst_max = v;
}
...
static void emit_regions(FILE* f, const RegionsEmitCtx* c, int hdr_only) {
    fprintf(f, "# schema_version=%s kind=%s comparison_id=%s "
               "fst_threshold=%.6g stronger_threshold=%.6g merge_gap_bp=%ld\n",
            SCHEMA_VERSION, hdr_only ? "HDR_regions" : "outlier_regions",
            c->comp_id, c->threshold_used, c->stronger_threshold, c->merge_gap_bp);
    fprintf(f, "region_id\tchrom\tstart\tend\tlength_bp\tn_windows\tn_variants_total\t"
               "fst_mean\tfst_median\tfst_max\tfst_min\t"
               "fst_threshold_used\tstronger_threshold_used\tcontains_strong_window\tclass");
...
static void print_usage(void) {
    fprintf(stderr,
        "fst_outlier_scan — genome-wide FST outlier + HDR scan (cichlid-paper style).\n"
        "\n"
        "Input:\n"
        "  --input <fst_windows.tsv>      Required. Per-window FST table.\n"
        "  --chrom_col <name>             default: auto (chrom/chromosome/chr/contig).\n"
        "  --start_col <name>             default: auto (start/win_start).\n"
        "  --end_col   <name>             default: auto (end/win_end).\n"
        "  --fst_col   <name>             default: auto (fst/FST/Fst_*/hudson_fst).\n"
        "  --nvar_col  <name>             optional (n_variants/n_sites).\n"
        "\n"
        "Thresholds:\n"
        "  --fst_threshold F              outlier flag at FST >= F (default 0.25).\n"
        "  --top_pct P                    use 99th-pct-style threshold from data;\n"
        "                                  P in [0,100]. Overrides --fst_threshold.\n"
        "                                  E.g. --top_pct 1 → top 1%% = 99th percentile.\n"
        "  --merge_gap_bp B               merge flagged windows ≤ B bp apart (default 10000).\n"
        "  --stronger_threshold F         HDR escalation threshold (default 0.30).\n"
        "  --stronger_windows <tsv>       optional. 10-kb-window FST table (same column\n"
        "                                  conventions); HDR requires overlap with at\n"
        "                                  least one row whose FST >= stronger_threshold.\n"
        "                                  Without it, HDR uses region max_FST.\n"
        "\n"
        ...
        "  --comparison_id <s>            e.g. wildA_vs_hatcheryB\n"
        "  --group_a <s>  --group_b <s>\n"
        "  --window_mode <s>              e.g. 15_variant, fixed_bp_10kb (free-text)\n"
        "  --genome_bp N                  if set, used to compute percent_genome_*\n"
        "\n"
        "Outputs (any subset, all TSV):\n"
        "  --out_windows <f>              flagged input windows above threshold\n"
...
int main(int argc, char** argv) {
    ...
    const char *chrom_col_name = NULL, *start_col_name = NULL,
               *end_col_name = NULL, *fst_col_name = NULL, *nvar_col_name = NULL;
    double fst_threshold = 0.25, stronger_threshold = 0.30;
    double top_pct = -1;
    ...
    const char *comp_id = "comparison", *group_a = "A", *group_b = "B", *window_mode = "unknown";
    long genome_bp = -1;
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (!strcmp(argv[i], "--fst_col")            && i+1<argc) fst_col_name = argv[++i];
        else if (!strcmp(argv[i], "--nvar_col")           && i+1<argc) nvar_col_name = argv[++i];
        else if (!strcmp(argv[i], "--fst_threshold")      && i+1<argc) fst_threshold = atof(argv[++i]);
        ...
        else { fprintf(stderr, "[fst_scan] Unknown arg: %s\n", argv[i]); print_usage(); return 1; }
    }
    if (!in_path) { print_usage(); return 1; }

    Tsv t;
    if (!tsv_load(in_path, &t)) { fprintf(stderr, "[fst_scan] empty input\n"); return 2; }
    fprintf(stderr, "[fst_scan] Loaded %d rows × %d cols\n", t.n_rows, t.n_cols);
    ...
    static const char* FST_ALIASES[]   = {"fst", "FST", "Fst", "hudson_fst", "fst_mean", NULL};
    ...
    int fst_idx   = fst_col_name   ? tsv_col_idx(&t, fst_col_name)   : tsv_find_col(&t, FST_ALIASES);
    int nvar_idx  = nvar_col_name  ? tsv_col_idx(&t, nvar_col_name)  : tsv_find_col(&t, NVAR_ALIASES);

    // Auto-detect Fst_<g1>_<g2> if no exact match and no --fst_col.
    if (fst_idx < 0 && !fst_col_name) {
        for (int j = 0; j < t.n_cols; j++) {
            if (!strncasecmp(t.col_names[j], "fst_", 4)) { fst_idx = j; break; }
        }
    }
    if (chrom_idx < 0 || start_idx < 0 || end_idx < 0 || fst_idx < 0) {
        fprintf(stderr, "[fst_scan] Missing required column. "
                "chrom_idx=%d start_idx=%d end_idx=%d fst_idx=%d\n",
                chrom_idx, start_idx, end_idx, fst_idx);
        tsv_free(&t); return 3;
    }
    fprintf(stderr, "[fst_scan] Using cols: chrom=%s start=%s end=%s fst=%s%s%s\n",
    ...
    for (int i = 0; i < t.n_rows; i++) {
        double fst; long s, e;
        if (!parse_d(t.cells[i][fst_idx], &fst)) continue;
    ...
    // Threshold (top_pct overrides absolute).
    double threshold_used = fst_threshold;
    if (top_pct >= 0 && top_pct <= 100) {
        double q = 1.0 - top_pct / 100.0;
        double* arr = (double*)malloc((size_t)nw * sizeof(double));
        int m = 0;
        for (int i = 0; i < nw; i++) if (isfinite(w[i].fst)) arr[m++] = w[i].fst;
        threshold_used = percentile_value(arr, m, q);
        free(arr);
        fprintf(stderr, "[fst_scan] --top_pct %.4g → threshold = %.6g\n", top_pct, threshold_used);
    } else {
        fprintf(stderr, "[fst_scan] fst_threshold = %.6g\n", threshold_used);
    }

    int n_flagged = 0;
    for (int i = 0; i < nw; i++)
        if (isfinite(w[i].fst) && w[i].fst >= threshold_used) { w[i].flagged = 1; n_flagged++; }
    fprintf(stderr, "[fst_scan] flagged %d / %d windows\n", n_flagged, nw);

    // Optional: stronger 10-kb windows for HDR.
    Window* sw = NULL;
    int nsw = 0;
    if (stronger_windows_path) {
        ...
        if (tsv_load(stronger_windows_path, &ts)) {
            ...
            int sf = fst_col_name   ? tsv_col_idx(&ts, fst_col_name)   : tsv_find_col(&ts, FST_ALIASES);
            if (sf < 0 && !fst_col_name) {
                for (int j = 0; j < ts.n_cols; j++)
                    if (!strncasecmp(ts.col_names[j], "fst_", 4)) { sf = j; break; }
            }
            if (sc < 0 || ss < 0 || se < 0 || sf < 0) {
                fprintf(stderr, "[fst_scan] --stronger_windows: missing required column\n");
            } else {
                sw = (Window*)malloc((size_t)ts.n_rows * sizeof(Window));
                for (int i = 0; i < ts.n_rows; i++) {
                    double fst; long s, e;
                    if (!parse_d(ts.cells[i][sf], &fst)) continue;
                    if (!parse_i(ts.cells[i][ss], &s)) continue;
                    if (!parse_i(ts.cells[i][se], &e)) continue;
                    if (fst < stronger_threshold) continue;
                    sw[nsw].chrom = strdup(ts.cells[i][sc]);
                    sw[nsw].start = s; sw[nsw].end = e; sw[nsw].fst = fst;
                    sw[nsw].n_variants = -1; sw[nsw].flagged = 1;
                    nsw++;
                }
                fprintf(stderr, "[fst_scan] stronger windows passing %.3f: %d\n",
                        stronger_threshold, nsw);
            }
            tsv_free(&ts);
        }
    }

    // Merge flagged windows into regions.
    ...
    for (int i = 0; i < nw; i++) {
        ...
        if (nr > 0
            ...
            region_push_fst(&regs[nr-1], w[i].fst);
        } else {
            ...
            regs[nr].fst_sum = 0;
            regs[nr].fst_min = INFINITY; regs[nr].fst_max = -INFINITY;
            regs[nr].contains_strong_window = 0;
            region_push_fst(&regs[nr], w[i].fst);
            nr++;
        }
    }
    fprintf(stderr, "[fst_scan] merged into %d outlier regions\n", nr);
    ...
    for (int i = 0; i < nr; i++) {
        if (sw && nsw > 0) {
            ...
            regs[i].contains_strong_window = (regs[i].fst_max >= stronger_threshold) ? 1 : 0;
        }
    }
    ...
    if (out_windows) {
        ...
        if (f) {
            fprintf(f, "# schema_version=%s kind=outlier_windows comparison_id=%s "
                       "fst_threshold=%.6g\n", SCHEMA_VERSION, comp_id, threshold_used);
            fprintf(f, "chrom\tstart\tend\tfst\tn_variants\tflagged\n");
            for (int i = 0; i < nw; i++) {
                if (!w[i].flagged) continue;
                fprintf(f, "%s\t%ld\t%ld\t%.6g", w[i].chrom, w[i].start, w[i].end, w[i].fst);
    ...
    // ── Build regions table and emit (top-level emit_regions; ctx-based) ──
    RegionsEmitCtx ctx = {
        .regs = regs, .nr = nr, .comp_id = comp_id,
    ...
    if (out_summary) {
        ...
        if (f) {
            ...
            fprintf(f, "  \"fst_threshold\": %.6g,\n", threshold_used);
            fprintf(f, "  \"fst_threshold_source\": \"%s\",\n",
                    (top_pct >= 0) ? "top_pct" : "absolute");
            ...
            fprintf(f, "  \"note\": \"Empirical FST outliers; not a neutral-calibrated selection scan.\"\n");
            fprintf(f, "}\n");
            fclose(f);
        }
    }
    ...
    for (int i = 0; i < nr; i++) free(regs[i].fst_values);
    free(regs);
